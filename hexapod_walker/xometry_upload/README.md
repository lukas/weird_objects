...
├── chassis_plate.stl          -- qty 2  (top + bottom hex plates, identical)
...
```bash
./run.sh hexapod_walker/prepare_xometry_upload.py
```
...
For the conservative builder: open `chassis_plate.stl`,
`coxa_bracket.stl`, `coxa_link.stl`, `femur_link.stl`, and
`tibia_link.stl` in your favourite STL viewer (MeshLab, PrusaSlicer,

...
# ---------------------------------------------------------------------------
# Optional servo visual meshes -- generated by hexapod_prototype.main()
# ---------------------------------------------------------------------------
# If the user has run ``hexapod_prototype.py`` (or ``build_all.py``), there
# will be servo_body.stl and servo_horn.stl in ``stl_prototype/``.  When
# present, the MJCF references them as scaled mesh visuals so the rendered
# scene shows the actual hobby-servo silhouette in every well/cradle.  When
# missing, we fall back to the simple primitive boxes used historically.
...
PITCH_ARM_T = 6.0     # MUST match make_coxa_link()'s arm_t
HIP_DROP = (-(HP.WELL_D / 2.0 + PITCH_ARM_T / 2.0) + HP.COXA_LIFT) * M

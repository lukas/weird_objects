...
- Sim plant uses the operator's MEASURED tibia (150 mm, commit a4beb8af 08-21; was 128) in physics, gait IK, and geometry checks — walk stance now ~169 mm tall. Every pre-08-22 checkpoint (incl. the download hierarchy) trained on the old 128 mm plant. MEASURED 08-22: the download hierarchy HARD-FAILS the interactive session gate at tibia-150 (falls on sit tilt_pitch + reverse-drive tilt_roll, fwd yaw -21.8 deg) while the matched control (same HEAD harness, only a4beb8af reverted) PASSES — the plant correction alone breaks the shipped answer, and DOWNLOAD_ANSWER's n=600 bulk numbers are old-plant facts. Evidence: `logs/ckpt_eval/plantgate_tibia150_session/`. Plant-150 re-hardening fix arms are spec'd but LAUNCH-BLOCKED: the rise/lower/walk semantics banks are RED at HEAD (14 FAIL; matched-control split: 4 tibia-150-caused incl. the walk gait-gate honest-vs-flag-leg orderings, 10 fail on both plants with onset in the 08-21/22 commit window — bisect/repair is the topmost agent-doable item; no stance/walk launch until green). Directionally consistent with the bench (learned stance tripped tilt_roll on hardware while the old sim passed) — hypothesis, not proof. The scripted tripod teacher is verified clean at the new plant (08-22 grid: 0.06-0.10 m/s x 4 headings, zero falls, slip/m 1.4-2.9, full fast profile).

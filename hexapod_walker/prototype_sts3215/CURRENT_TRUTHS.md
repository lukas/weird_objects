...
- Sim plant uses the operator's MEASURED tibia (150 mm, commit a4beb8af 08-21; was 128) in physics, gait IK, and geometry checks — walk stance now ~169 mm tall. Every pre-08-22 checkpoint (incl. the download hierarchy) trained on the old 128 mm plant. MEASURED 08-22: the download hierarchy HARD-FAILS the interactive session gate at tibia-150 (falls on sit tilt_pitch + reverse-drive tilt_roll, fwd yaw -21.8 deg) while the matched control (same HEAD harness, only a4beb8af reverted) PASSES — the plant correction alone breaks the shipped answer (differential claim; both arms shared the then-corrupted harness, absolute numbers predate the 08-22 convention repair), and DOWNLOAD_ANSWER's n=600 bulk numbers are old-plant facts. Evidence: `logs/ckpt_eval/plantgate_tibia150_session/`. BANK BREAK ROOT-CAUSED + REPAIRED 08-22: bisect pinned the 08-21/22 bank collapse (43 FAIL full-file) to 30660b51's absolute-tibia knee convention + new +19/+28-absolute default stand home leaking into the femur-relative sim via desired_deg/_leg_ik/standing_pose; fixed at the boundary by `linux_control/sim_gait_compat.py` (sim-relative adapters, hardware untouched) + a `_default_plant_deg` guard (hardware DEFAULT stand home is near-singular at +19/+9 rel and is NOT adopted; genuinely captured plants convert). Residue: 7 FAIL, all reproduced at a4beb8af => true tibia-150 recalibration items (stale 128 mm rise ref npz -> rise_valid_plant/score_replay; rise_rock; trans_drag; getup_honest_ordering; recover_floor_rungs; fastprof_obeying). Walk banks GREEN => `cw-dep-bcgait1-plant150-1` unblocked; stance fix arm still blocked on the rise-family residue. `cw-dep-bcgait4-phasedir1` trained ON the corrupted sim (its snapshot contains 30660b51): its RL-degradation verdict is env-confounded and needs a re-run on the repaired sim before any phase-RL conclusion. Directionally consistent with the bench (learned stance tripped tilt_roll on hardware while the old sim passed) — hypothesis, not proof. The scripted tripod teacher is verified clean at the new plant (08-22 grid: 0.06-0.10 m/s x 4 headings, zero falls, slip/m 1.4-2.9, full fast profile).

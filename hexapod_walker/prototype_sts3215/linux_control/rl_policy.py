"""On-robot RL policy runner: STAND UP / LOWER buttons (web UI).

Runs the sim-trained raw-joint PPO policy (exported to plain numpy
weights by ``rl_move/sim/export_policy_np.py`` — no torch on the board)
in the exact conventions it was trained with:
...
WEIGHTS_PATH = _HERE / "rl_policy_weights.json"
HZ = 25.0
DT = 1.0 / HZ
...
def preflight(bus, mode: str) -> tuple[bool, str, dict]:
    ...
    if exp is None:
        return False, err, {}
    dq = np.abs(q_deg - exp)
    tol = STAND_START_TOL_DEG if mode == "stand" else LOWER_START_TOL_DEG
    details = {
        "roll_deg": round(roll, 1), "pitch_deg": round(pitch, 1),
        "max_pose_delta_deg": round(float(np.max(dq)), 1),
        "pose_tol_deg": tol,
    }
    if mode == "stand" and (abs(roll) > PREFLIGHT_MAX_TILT_DEG
...
def run_policy_move(drive, mode: str, *, on_progress=None,
    ...
    ``mode`` is "stand" or "lower".
    """
    assert mode in ("stand", "lower")
    ...
    for i in range(n_ticks):
        ...
        goal = TaskGoal(roll_ref=0.0, pitch_ref=0.0,
                        height_ref=_height_ref(mode, t), unload_leg=None)
        ...
        if i % 5 == 0:
            ...
            on_progress({
                "msg": (f"{mode} {phase} t={t:4.1f}s "
                        f"href={goal.height_ref * 1000:+.0f}mm "
                        f"maxI={max_cur:.2f}A"),

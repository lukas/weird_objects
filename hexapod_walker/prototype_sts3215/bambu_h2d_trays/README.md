# Bambu H2D Carbon (single nozzle) trays — hexapod prototype

Generated for a **325 x 320 x 325 mm** build volume.
...
- `plate_03_rigid_coxa_links_a.stl` (PLA/PETG rigid): 6 x `coxa_hip_bracket.stl`, 6 x `coxa_yaw_hub.stl`, 6 x `servo_clamp_cap.stl`, 1 x `yaw_bearing_cap.stl`, 6 x `yaw_servo_retainer.stl`
- `plate_03_rigid_coxa_links_b.stl` (PLA/PETG rigid): 5 x `yaw_bearing_cap.stl`
- `plate_04_rigid_femur_sockets.stl` (PLA/PETG rigid): 6 x `femur_hip_yoke.stl`, 6 x `femur_knee_bracket.stl`, 6 x `servo_clamp_cap.stl`

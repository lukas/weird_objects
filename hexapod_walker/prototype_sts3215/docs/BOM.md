...
| 1 | M3 x 10 SHCS (`91290A114`) | M3 x 10 mm socket-head cap screw, black-oxide steel.  **2 used (Aug 2026):** the switch_holster mount bolts.  The 4 chassis_top → brass-standoff bolts moved to M3 × 8 when the top plate was halved to 2 mm (an x10 tip would bottom out in the standoff).  Deck-column tray bolts are retired.  Buy a 50-pack so you have spares. | [McMaster 91290A114](https://www.mcmaster.com/91290A114/) |
...
| 1 | M3 x 8 SHCS (`91290A113`) | M3 x 8 mm socket-head cap screw, black-oxide steel.  **~46 used (Aug 2026):** **24 sandwich-joint clamp-cap bolts** + **18 yaw_bearing_cap join screws** + **4 chassis_top → brass-standoff top bolts** (moved from M3 × 10 when the top plate was halved to 2 mm).  Buy a 100-pack so you have spares. | [McMaster 91290A113](https://www.mcmaster.com/91290A113/) |
...
| M3x10 SHCS | 91290A114 | 2 | switch_holster heat-set inserts |
...
|---:|---|---|---|---:|---:|---:|---|
| 1 | `chassis_top.stl` | PLA / PETG | 0.2 mm | 25% gyroid | 4 | ~2 h | Top hex; 4× `CHASSIS_STANDOFF_HOLES_XY` (±31.1) take sandwich standoffs below + magnet posts above. |
| 1 | `chassis_bottom.stl` | PLA / PETG | 0.2 mm | 25% gyroid | 4 | ~2 h | Single merged Jun 2026 part: genuinely FLAT-bottom 200 mm hex plate with a folded-in ~8 mm-thick flat floor carrying the 6 yaw-servo cradles + upward yaw-bearing tower. Prints face-DOWN, no supports. Aug 2026 rev: one open 18 × 28 mm harness port per leg (ribs removed) + a 14 × 22 mm battery XT60 pass-through at (48, 0); late-Aug 2026: the 6 corner power-Wago tray wall sets are printed into the top face (the separate `wago_mount` trays are retired). Two shorty LiPos velcro'd under the belly (centre slot pair = safety strap); data Wagos under the belly at the hex vertices. |
| 1 | `switch_holster.stl` | PLA / PETG | 0.2 mm | 25% gyroid | 3 | ~0.3 h | Snap-in holster for the anti-spark on/off switch. Bolts to chassis_top's +X edge via 2× M3 × 10 SHCS into 2 heat-set inserts. |
| 1 | `extra_stl/round_mount_plate_115_with_leg_holes.stl` | PLA / PETG (print it — the SVG cut variant loses the registration bosses) | 0.2 mm | 20% gyroid | 3 | ~0.5 h | Round Ø115 magnet-held board (matches the chassis_top disc) for Uno Q + breakout on top, 3.3 V Wago underneath. Late-Aug 2026: 4 underside bosses socket the Ø8 magnet tops (shear registration — magnets only carry pull) and the legacy 49.5 mm bolt square is dropped; keeps the live ±31.1 square + 6 platform leg holes, 2 Ø8 wire ports + E/W zip-tie slots. Generate with `tools/make_xtool_hex_raised_platform.py`. |
| 1 | `extra_stl/hex_raised_platform_110_h28_screen.stl` | PLA / PETG | 0.2 mm | 15% gyroid | 3 | ~0.8 h | Raised platform — screen on the top face, held by 4× M2 self-tappers in Ø1.8 corner pilot holes; its 8-wire Uno pigtail enters through the 24×5 slot behind the panel's +X edge (28 mm legs, shortened from 72 in the late-Aug 2026 design review — less lever on the magnet-held plate, ~44 mm CoG drop; MPU is on chassis_top near the centre, not under this plate). |
| 12 | `servo_clamp_cap.stl` | MJF PA12 (PLA OK) | 0.2 mm | solid | 3 | ~0.2 h ea | Sandwich-joint clamp cap — 1 per hip + knee joint (2 per leg × 6 legs). Bolts to the cradle ±X wall ends with 2× M3 × 8 SHCS self-tapping into the cradle pilots. |
| 6 | `coxa_link.stl` | PLA / PETG | 0.2 mm | 30% gyroid | 4 | ~4 h total | ONE piece (Aug 2026 merge): yaw turntable hub (rides the touching 6805 pair) + hip fixed side; 5 head-access shafts reach the yaw horn screws through the empty hip servo well. Print on its SIDE (yaw axis horizontal, cradle end wall on the bed). |
...
| 6 | `femur_link.stl` | PLA / PETG | 0.2 mm | 35% gyroid | 4 | ~8 h total | The WHOLE femur, one printed part (Jul 2026 merge) — hip moving yoke + solid Ø14 spar + knee fixed side. No CF tube, no socket, no roll pin. Print yoke spine-down / spar horizontal; support the knee servo well through its open back. |
| 6 | `tibia_knee_yoke.stl` | PLA / PETG | 0.2 mm | 35% gyroid | 4 | ~4 h total | Knee moving yoke (symmetric clevis — both arms bolt to a disc horn) + Ø8 CF-tube socket. |
| 6 | `foot_boot.stl` | TPU 95A (required — PLA has no grip or stretch) | 0.25 mm | 100% (TPU) | 3 | ~0.8 h total | TPU boot pressed over the tibia Ø8 CF-tube end (Aug 2026 — replaces `tibia_foot_fitting` + `foot_pad` + the hinge pin). Ø7.7 bore = 0.3 mm interference. Print tip-face on bed, bore up — no supports. |

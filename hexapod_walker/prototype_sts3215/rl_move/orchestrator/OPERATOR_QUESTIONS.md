...
- status: OPEN (no operator pause; proceeding — the fork itself needs a deep-model dig-in cycle, not an operator answer, per the no-pauses rule. Flagged for the watcher's model-tiering handoff.)

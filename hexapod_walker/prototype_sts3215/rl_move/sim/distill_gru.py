...
def collect(env, teachers: dict, episodes_by_mode: dict[str, int],
    ...
    for mode, n_ep in episodes_by_mode.items():
        ...
        while done_eps < n_ep:
            ...
            done_eps += 1
        print(f"[distill-gru] {mode}: {n_ep} episodes "
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--episodes", type=int, default=400,
                    help="total demo episodes, split per the r3 diet")
    ...
    episodes_by_mode = {m: max(1, round(args.episodes * w))
                        for m, w in DIET.items()}
    teachers = {"walk": (walk_teacher, n_walk),
                "stance": (stance_teacher, n_stance)}
    episodes = collect(env, teachers, episodes_by_mode,
    ...
    print(f"[distill-gru] final actor RMS {np.sqrt(actor_mse):.4f} action "
    ...
    print(f"[distill-gru] saved {args.out}")
    return 0

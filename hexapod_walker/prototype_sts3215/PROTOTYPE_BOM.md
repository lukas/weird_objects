...
> `battery_holder`).  A bolt-on **`spider_carapace`** domed shell (with the
> classic 8-eye spider face) caps the stack as a 3rd deck level on four more
> M3 standoffs at `DECK_COLUMN_XY`; its open skirt + rear window leave every
> wire exit and the buck converter vented.
...
| 1 | LiPo battery | 3S, 25C or higher, XT60 connector, up to **138 × 46 × 24 mm** (the envelope the CAD reserves in the inter-plate bay — Jul 2026 battery-fit rework). The 11.1 V nominal (12.6 V full) drives the STS3215 bus V+ rail directly. Velcro-strapped to the top of `chassis_bottom` in the inter-plate gap. | [Amazon: 3S 5200mAh 25C lipo XT60](https://www.amazon.com/s?k=3S+5200mAh+25C+lipo+XT60) |
...
| 1 | Power distribution board (the "bus bar") | **Matek PDB-XT60** drone power-distribution board (36 × 50 mm, 11 g w/ XT60): 6 output pad pairs at 15 A continuous each + 1 VCC/GND pad pair, XT60 input socket, 9–18 V (3S OK).  Solder the 6 per-leg 16–18 AWG branches to the output pads and the buck feed to the VCC/GND pads.  Rated 4× above the ~3.7 A worst-case branch draw; replaces a heavy brass bus bar (~300 g+) at no capability loss.  Everywhere these docs say "bus bar", this is the part.  **Required** because the 18 servos cannot be power-daisy-chained through the ~3 A Molex 5264 pins (see `firmware/WIRING.md` §6).  (Ignore its tiny on-board 5 V/12 V BECs — the Uno Q stays on the XINGYHENG buck.) | [Matek PDB-XT60](https://www.jsumo.com/matek-pdb-xt60-wbec-5v-and-12v) / [Amazon: matek pdb-xt60](https://www.amazon.com/s?k=matek+pdb+xt60) |
...
| 1 | M3 nyloc nuts | 100-pack nylon-insert lock nuts. **6 used (Jul 2026 F/F standoff switch):** one per foot-pad hinge pin.  The 8 standoff-retention nylocs are gone — the chassis and lowest deck standoffs are now FEMALE-FEMALE brass, bolted from both ends (M3 x 14 SHCS up through chassis_bottom; M3 x 8 SHCS up through chassis_top for the deck columns).  Servo retention uses no nuts either (hip: 4× M2.5 into the END-face case holes + clamp cap; knee: clamp cap + lip; yaw: retainer strap). | [Amazon: M3 nyloc lock nut 100 pack](https://www.amazon.com/s?k=M3+nyloc+lock+nut+100+pack) |
| 1 | M3 heat-set inserts (`94459A130`) | McMaster knurled brass M3 heat-set insert, Phi 4.0 mm pilot, Phi 5.7 mm OD, 5.0 mm length. **18 used (Jun 2026 deck + carapace):** 8 in the printed deck trays for the board-mount bosses (4 `uno_q_tray` + 4 `buck_tray`) + 4 in the imu_pad (MPU-6050 mount) + 2 in chassis_top's printed bosses (switch_holster) + 4 in the `spider_carapace` feet (dome → standoff mount).  (The 24 sandwich-joint clamp-cap bolts self-tap and use NO inserts; the 24 M2.5 servo body-retention screws — hip cradles only since the Jul 2026 one-piece femur — thread directly into the servo's own metal end-face case holes, also no inserts.)  Installed with a soldering iron at ~220 deg C, light downward pressure, ~10-15 s per insert, then cool ~30 s; an M3 x 8 SHCS (deck board / IMU pad) or M3 x 10 SHCS (switch_holster) threads into the brass instead of self-tapping into plastic. | [McMaster 94459A130](https://www.mcmaster.com/94459A130/) |
| 1 | M3 x 10 SHCS (`91290A114`) | M3 x 10 mm socket-head cap screw, black-oxide steel.  **14 used (Jul 2026):** 2 switch_holster mount bolts (DOWN through the holster ear into chassis_top boss inserts) + 4 chassis_top → brass-standoff bolts (DOWN from above chassis_top into the F-F standoff female top threads on the 44-mm-radius diagonal pattern) + **8 deck standoff-column bolts (DOWN through each tray's column bosses into the brass standoff female threads — 4 at `uno_q_tray`, 4 at `buck_tray`)**.  (The Jun 2026 chassis-plate merge eliminated the 12 print-split join screws — `chassis_bottom` is now a single printed part.)  Buy a 50-pack so you have spares. | [McMaster 91290A114](https://www.mcmaster.com/91290A114/) |
...
| 1 | M3 x 8 SHCS (`91290A113`) | M3 x 8 mm socket-head cap screw, black-oxide steel.  **58 used (Jul 2026):** 8 deck board-mount bolts (4 Uno Q + 4 buck, DOWN through each PCB into the tray's M3 heat-set inserts) + 4 imu_pad (MPU-6050 mount) + **24 sandwich-joint clamp-cap bolts (2 per hip + knee clamp cap, self-tapping into the cradle ±X wall-end pilots; 12 clamp caps robot-wide)** + **18 yaw_bearing_cap join screws (3 per leg × 6, self-tapping DOWN into the split bearing-tower pilots to capture the spaced 6706 pair)** + **4 deck-column bottom bolts (UP through chassis_top into the lowest F-F deck standoffs — replaces the old nylocs)**.  Buy a 100-pack so you have spares. | [McMaster 91290A113](https://www.mcmaster.com/91290A113/) |
...
| 1 | M3 standoffs (chassis) | M3 x 32 mm FEMALE-FEMALE brass standoffs, pack of 10-20.  **4 used** on the 44-mm-radius diagonal pattern (`CHASSIS_STANDOFF_HOLES_XY` = (±31.1, ±31.1); Jul 2026 battery-fit rework moved them off (±35, 0)/(0, ±35) so the 138 × 46 mm LiPo has a clear lane) clamping chassis_top to chassis_bottom across the CHASSIS_GAP = 32 mm inter-plate gap.  Jul 2026 F/F switch: an M3 × 14 SHCS enters UP from below chassis_bottom's −6 mm floor face into the standoff's bottom female thread (the plate + floor stack is 8 mm — too thick for an M-F male stud + nyloc); an M3 × 10 SHCS drops DOWN from above chassis_top into the female top.  Re-verify lengths whenever CHASSIS_GAP changes. | [Amazon: M3 32mm standoffs female female brass](https://www.amazon.com/s?k=M3+32mm+standoffs+female+female+brass) |
| 8 | M3 standoff columns (deck) | M3 brass standoffs for the stacked electronics deck on the `DECK_COLUMN_XY` (±41, ±33) pattern above chassis_top.  **4 at `DECK_LEVEL_1_STANDOFF_H` (16 mm, chassis_top → `uno_q_tray`, FEMALE-FEMALE)** + **4 at `DECK_LEVEL_2_STANDOFF_H` (22 mm, `uno_q_tray` → `buck_tray`)**.  Jul 2026 F/F switch: the lowest columns take an M3 × 8 SHCS UP through chassis_top into their bottom female threads (replaces the old male stud + nyloc); each upper level bolts down with an M3 × 10 SHCS.  Buy a mixed standoff assortment that covers both lengths. | [Amazon: M3 standoff assortment female brass](https://www.amazon.com/s?k=M3+standoff+assortment+female+brass) |
...
| M3x8 SHCS | 91290A113 | 4 | deck standoff column bottom bolts (M3 x 8 SHCS up through chassis_top) |
...
|---:|---|
| 1 | `chassis_top.stl` |
| 1 | `chassis_bottom.stl` (single merged Jun 2026 part: genuinely FLAT-bottom 200 mm hex plate with a folded-in ~8 mm-thick flat floor carrying the 6 yaw-servo cradles + upward yaw-bearing tower + tray bosses; prints face-DOWN, no supports) |

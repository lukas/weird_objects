...
| `k_loadslip_excess` | 0 | direct anti-skate charge (fast anti-skate V5, operator order fb_20260820T075230_4a90c6: both steer6/steer7 fast-gait forks kept skating because the loadslip GATE only zeroes income — a skater collecting non-velocity reward is complained about, never charged): −k · max(`walk_loadslip_ratio` − `loadslip_ok`, 0) per commanded tick, the same episode-accumulated ratio the gate/harness score. Additive penalty, never shrunk by income gates; composes with `walk_loadslip_gate`. Info: `reward_loadslip_excess`. Semantics: `test_walk_fastprof_mdp.test_loadslip_excess_*`. |

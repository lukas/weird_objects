...
| `--recover-population-id` / `--recover-population-member` / `--recover-population-runs` | off | Best-of-N seeded recovery cohort. Each independent learner starts from the same curriculum stage. The first member whose deterministic promotion assay passes the frontier AND the fresh full retention suite publishes a policy/optimizer + curriculum candidate to W&B. Member 0 elects the earliest candidate; every member verifies and loads that exact checkpoint before collecting another rollout. The next stage cannot be elected until every roster member ACKs the same checkpoint hash. Parent hashes reject candidates learned on stale pre-sync branches. A candidate named Bk is the retained checkpoint that unlocks Bk (so B1 means B0 passed and B1 is now the frontier). Use unique run names in fixed member order and distinct `--seed` values. |
...
Population cohorts additionally log candidate, winner, ACK/adopted bucket,
winner member, and sync count under `RECOVER_POPULATION/*`.

...
**verdict**: DEAD - INFRASTRUCTURE (no behavioral read): killed ~5 min in (~200k steps, W&B 08k9lmkm finalized cleanly at 17:02) as collateral of the -fullpace2 attempt-1 launch, which raced the pod-freeness check onto train-0 (source-run's-pod default) and repaired away this trainer. Not a training failure. Relaunched once per DEAD-retry rule as -acq8m-r1 on a free pod, same spec verbatim.

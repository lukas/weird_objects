...
**verdict**: Crashed at launch, 0 training steps (~1s runtime, exit before first rollout): the respec inherited a stale --obs-pad-transplant=4 flag from the getup2-r1 obs-widening fix, but this continuation warm-starts from cw-getup3 itself (same joint_walk task, obs already 72-wide, widened-by-0) so the transplant's own safety check raised SystemExit("--obs-pad-transplant 4 but obs widened by 0 (72 -> 72)"). Mechanical launch bug, not a science result — no verdict on the BC-anchor-getup recipe itself. Relaunched corrected as cw-getup3-c2 (--obs-pad-transplant=0, otherwise identical).

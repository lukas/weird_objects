...
**verdict**: Never trained: launcher REFUSED twice on pod code-marker churn, then two same-name retries (mine included) cloned a malformed ledger row carrying the any19 roster flags (--recover-population-id/runs) with any20 ids and died at argparse (--run-name must match roster). No W&B run ever created; id 7ab95ed5 unused. Cohort superseded by cw-recover-any21-pop3 per fb_20260818T002830 fallback.

...
**verdict**: CANARY FAIL - INFRASTRUCTURE: reward.rise_ref_path pointed at rise_ref_mesh_tuckrise15.npz, never committed to git (only tuckrise45.npz landed in fa1bccc5) -> worker crashed on env reset (FileNotFoundError), 0 training steps, zero information. No relaunch of the 15mm variant (file no longer exists). Ledger backfill only (W&B OUTCOME note already recorded this at the time).

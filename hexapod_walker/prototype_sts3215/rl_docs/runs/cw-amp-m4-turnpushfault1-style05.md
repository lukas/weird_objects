...
**verdict**: LAUNCH_CRASH, self-inflicted: --init-from-source resolved to the --from run's own checkpoint (ppo_goal_cw_amp_m4_turnfault1_style05.zip, already 93-dim with the fault pad baked in) instead of the intended --parent (cw-amp-m3-turnpush1-style05-acq1-r2's 75-dim turn+push checkpoint) -- respec's --init-from-source flag ignores --parent and always warm-starts from the SOURCE run, a launcher semantics error on this cycle's part, not a code bug. --obs-pad-transplant 18 then padded an already-93-dim obs to 93 ('widened by 0'), the trainer refused immediately, zero steps trained (process died within seconds of wandb init). Killed the stuck launch_run.py verification loop manually (PID left polling a dead process). Corrected relaunch (--arg='--init-from=...' explicit, no --init-from-source) follows this cycle.

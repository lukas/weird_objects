...
| `eval/<walk-mode>/direction_err_deg`, `direction_err_p90_deg` | **SECONDARY DIAGNOSTIC since 2026-08-29** (operator directive fb_20260829T141858_9421cd): mean / p90 angle between actual XY velocity and the current nonzero command, over ticks moving at least 0.01 m/s; 0° is aligned, >90° is wrong-way. This tick-level statistic is dominated by honest tripod stride sway (measured floors: ~35° primitive/25 Hz; the clean scripted teacher on mesh/100 Hz reads mean 13.5° / p90 35.9° while its NET course is 1.3° off) — use it for stride-oscillation/wrong-way-spike triage, never alone as the joystick-following headline. |
...
| `course_err_{1s,2s}_{mean,med,p90}_deg` (harness ep + `eval/<dr>/<mode>_<tag>/course_err_*_med` W&B medians) | **PRIMARY command-following read since 2026-08-29**: course error of rolling 1 s / 2 s windows — angle between the window's NET XY displacement and the command INTEGRATED over the same window (time-averaged command: robust to mid-window switches; windows straddling a stop tick or a command reversal that cancels >50% of the commanded distance are excluded). Angle stats only over windows with net motion ≥ 0.01 m/s·window. Teacher baseline (mesh/100 Hz, probe_dir_floor 2026-08-29): med 1.2–2.2°, p95 ≤ 5.2°. Implementation: `eval_checkpoint.windowed_course_stats`. |
| `wrong_course_frac_{1s,2s}` | fraction of motion-valid windows with course error >90° — windowed wrong-way travel, the meaningful replacement for charging every lateral half-stride |
| `course_speed_ratio_{1s,2s}_med`, `course_motion_valid_frac_{1s,2s}` | along-command net displacement / commanded distance per window (parking ~0, full completion 1; teacher on mesh/100 Hz measures ~0.39 — the scripted gait cannot reach 0.08 m/s under the slew contract) and the fraction of commanded windows clearing the motion floor. ALWAYS read these next to the course error: they are what keeps a parked robot from passing the windowed angle metric. |

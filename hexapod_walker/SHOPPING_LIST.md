...
All files live under `hexapod_walker/stl_prototype/`. Filenames are
...
| 7 | `femur_link.stl` | **6** | PLA / PETG | 0.2 mm | 30% gyroid | 4 | ~ 5 h total | Print on its broadest face — the I-beam slot opens UP, the hip-pad lies flat on the bed. |
...
> in `hexapod_walker/xometry_upload/` with a `manifest.csv` and a
...
| Qty | Part | Why | Search link |
|---:|---|---|---|
| 1 | **M3 socket-head cap screw + nut + washer assortment kit (~ 500 pieces, 6 / 8 / 10 / 12 / 16 / 20 mm lengths, A2 stainless)** | Simpler than buying lengths separately. Use 8 mm for servo tabs, 12 mm for chassis-spacer bolts, 16 mm for coxa-bracket → chassis, 20 mm for the rare longer reach. | [Amazon search: "M3 stainless screw kit assortment"](https://www.amazon.com/s?k=M3+stainless+screw+kit+assortment) |
| 1 | **M3 nylon-insert (nyloc) lock nut, ~ 100 pieces** | Use these on every joint that sees vibration, especially the coxa-bracket → chassis bolts and any joint inside the leg. | [Amazon search: "M3 nyloc lock nut 100 pack"](https://www.amazon.com/s?k=M3+nyloc+lock+nut+100+pack) |
...
The script `hexapod_walker/_verify_prototype.py` re-checks the
...
```bash
./run.sh hexapod_walker/_verify_prototype.py
```
